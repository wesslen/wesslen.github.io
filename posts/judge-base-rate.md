...
A bank deploys a payments assistant that handles 50,000 customer conversations a month. An LLM judge reviews every transcript for the failure the bank fears most: the agent waving through a payment that is actually a scam. The monitoring dashboard reads 0.2% flagged, and the number has been stable for three months.
...
Human-confirmed cases then feed back into the gold set. Every confirmed failure becomes a label; enough labels turn a novel tactic into a known one and move it from the judge's beat to the classifier's. Over months, the classifier absorbs the head of the distribution while the judge and humans patrol the tail.
