...
![Five-phase pipeline: Scope → Attack Execution → Triage → Reporting → Retest, with an orange loop-back arrow from Retest to Scope labeled 'Not a Certification'; an independent tester figure above the pipeline, a crossed-out developer figure blocked by an orange barrier on the left](../img/adversarial-workflow-header.png)
*Five phases produce the artifacts — structural independence between the tester and the developer is what makes them controls rather than documentation.*
