...
That's not a rounding difference. It's the basis on which the field has been comparing models, training new agents on the "successful" trajectories, and publishing progress claims.

The architectural finding compounds this. Upgrading WebVoyager's backbone from GPT-4o to GPT-5.2 drops the FPR from 45% to 10%, but raises the false negative rate from 24% to 44%, and Cohen's κ improves only modestly. You're shifting which direction the error goes, not fixing the underlying problem. The Universal Verifier's advantage is architectural, not model-size-dependent.
...
> [!IMPORTANT]
> The Universal Verifier uses GPT-5.2 as its backbone. CUAVerifierBench (the human-labeled evaluation dataset) is open-sourced, enabling practitioners to test their own verifier designs against human labels. But practitioners using different backbone models will get different error distributions. The design principles transfer; the specific performance numbers don't.

## Four principles, one root

The Universal Verifier is built around four design principles. Rubric quality is where most of the leverage lives; the paper estimates it accounts for roughly half the Cohen's κ gains across the 96 experiments tracked in Figure 1.
...
The fix is architectural: generate the rubric from the task description alone, before seeing the trajectory. Score separately. Then run a two-pass hallucination check, scoring once with screenshot evidence and once without, and surface discrepancies between the two.
...
The fourth principle is more technical. Both WebVoyager and WebJudge assess screenshots by passing them in bulk: all of them for WebVoyager, a ranked top-50 for WebJudge. This creates a needle-in-a-haystack problem on longer trajectories, where critical state changes buried at step 37 of 80 get lost in context. Universal Verifier scores each screenshot against every rubric criterion to produce a relevance matrix, then groups the most relevant screenshots per criterion for focused analysis. The verifier attends to all evidence but focuses attention by what each criterion needs to see; this is why a stronger backbone doesn't close the gap. The problem isn't model intelligence, it's missing context.

## 70% in 5% of the time — and what it costs

The paper includes an experiment worth examining closely. An auto-research agent using Claude Opus 4.6 and Claude Code was given the same task as the human expert: design a verifier, iterate against CUAVerifierBench, maximize Cohen's κ without increasing FPR.[^5] Starting from blank prompts, it reached approximately 70% of expert-level performance in roughly 5% of the time: one day versus three weeks, across 96 total experiments.
...
Those step-function improvements were structural decisions: separating rubric generation from scoring, separating process rewards from outcome rewards, designing the two-pass hallucination check. The AI agent could optimize within an established architecture. It couldn't recognize that the architecture needed to change.

This is the third time this series has hit the same pattern. Part 1 found that MolmoWeb's synthetic teacher outperformed human annotators because the teacher had structured access to page semantics (AxTree element labels) that the trained agent won't have at runtime. The teacher's advantage was what it could *see* at the architectural level. Here, the human expert's advantage over the automated researcher is the same kind of structural visibility: the ability to recognize that generating and scoring a rubric in one pass is a different epistemic act than generating it independently.
