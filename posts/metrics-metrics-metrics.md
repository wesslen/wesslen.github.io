...
They call this construct validity, and it's a harder problem than it sounds. A 2025 paper by [Microsoft Research](https://arxiv.org/abs/2502.00561) argues — convincingly, in my view — that GenAI evaluation is fundamentally a social science measurement challenge rather than an engineering one.[^5] The distinction matters because engineering teams tend to assume that if you can run the test, you're measuring what you want. Social scientists know you have to verify that assumption.
...
![3PL IRT equation hand-drawn and annotated: P(theta) = c + (1-c) * sigma(a * (theta - b)). Four annotation arrows: c = GUESSING FLOOR (probability of correct answer by chance); theta = MODEL ABILITY (what we are estimating); a = DISCRIMINATION (how sharply item separates ability levels); b = ITEM DIFFICULTY (ability level where item is most informative); sigma = LOGISTIC FUNCTION. Bottom annotation: RAW ACCURACY COLLAPSES ALL FOUR PARAMETERS INTO ONE NUMBER.](../img/irt-3pl-annotated.png)
...
[^4]: [Akhtar et al. (2026), "When AI Benchmarks Plateau: A Systematic Study of Benchmark Saturation"](https://arxiv.org/abs/2602.16763) (arXiv:2602.16763). The study analyzes 60 widely used benchmarks and finds nearly half exhibit saturation — top models become statistically indistinguishable on aging benchmarks because they have learned to solve the specific items rather than the underlying skill, with saturation rates increasing as benchmarks age.
...
[^8]: [Ye et al. (2024), "Justice or Prejudice? Quantifying Biases in LLM-as-a-Judge"](https://arxiv.org/abs/2410.02736) (arXiv:2410.02736) documents 12 systematic biases including verbosity bias, positional (primacy/recency) bias, format bias, authority bias, and self-preference bias. The paper finds positional bias flip rates of 20–40% in pairwise comparisons — a serious problem for benchmarks like LMSYS Chatbot Arena that rely on head-to-head preference judgments.
[^9]: A nifty approach to LLM judge reliability: [*Who Judges the Judge? LLM Jury-on-Demand: Building Trustworthy LLM Evaluation Systems*](https://arxiv.org/abs/2512.01786) from researchers at Wells Fargo.
