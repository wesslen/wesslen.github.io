...
![Levels of Automation in Banking AI: horizontal scale from 1 (HUMAN DOES EVERYTHING) to 10 (COMPUTER ACTS AUTONOMOUSLY). Four use cases marked above: AML Transaction Monitoring at levels 2–3 (HUMAN INVESTIGATES ALERTS, blue), Credit Underwriting at 4–5 (HUMAN APPROVES OR REJECTS, blue), Fraud Decisioning at 6–7 (HUMAN CAN OVERRIDE, orange), HFT Execution at 9–10 (POST-HOC REVIEW ONLY, red). A wide orange bracket below spans the full range labeled: SR 26-2: ONE EFFECTIVE CHALLENGE EXPECTATION ACROSS ALL OF THIS.](../img/loa-levels.png)
*Levels of Automation (Sheridan-Verplank 1978 / Parasuraman et al. 2000). Banking operates across the full range; the same supervisory expectation applies to all of it.*
...
![Three-row diagnostic matrix illustrating Parasuraman & Riley's misuse/disuse/abuse taxonomy applied to banking: MISUSE row (orange) shows overreliance producing commission errors — credit officer approves loan despite contradicting file; DISUSE row (blue) shows underutilization producing omission errors — AML analyst in rote-rejection mode on 96%-false-positive queue; ABUSE row (red) shows misapplication producing structural mismatch — GenAI deployed without level-appropriate governance. Bottom annotation reads: SR 26-2 applies one supervisory expectation to all three.](../img/automation-bias-banking-header.png)
*Parasuraman & Riley (1997): three failure modes, three error types, three different mitigations. SR 26-2 addresses none of them by name.*
