...
![Multi-agent attack propagation diagram (MAESTRO threat model): a horizontal chain of four boxes — ORCHESTRATOR → AGENT A → AGENT B → AGENT C — each holding credentials. An orange zigzag prompt injection arrow strikes Agent A from above; orange dotted arrows labeled INJECTED CONTEXT propagate right through shared context to Agent B and Agent C. Red warning triangles mark ESCALATED PERMISSIONS at each handoff. A confused stick figure at the end asks 'WHICH ONE BROKE?' A wide red BLAST RADIUS box spans all agents. Bottom annotation: 'Authentication proves identity. It says nothing about intent.'](../img/a2a-risks-maestro.png)
*CSA MAESTRO threat model applied: a single injected instruction propagates across agent handoffs, escalating permissions at each step. The blast radius from one compromised agent spans the entire chain.*
...
[^10]: NIST AI Agent Standards Initiative (February 17, 2026), [nist.gov](https://www.nist.gov/artificial-intelligence/ai-agent-standards-initiative); CAISI Request for Information on Securing AI Agent Systems (January 2026), [nist.gov](https://www.nist.gov/news-events/news/2026/01/caisi-issues-request-information-about-securing-ai-agent-systems); NCCoE draft concept paper *Accelerating the Adoption of Software and AI Agent Identity and Authorization* (February 2026), [nccoe.nist.gov](https://www.nccoe.nist.gov/sites/default/files/2026-02/accelerating-the-adoption-of-software-and-ai-agent-identity-and-authorization-concept-paper.pdf). The paper names MCP/OAuth 2.1 as the primary authorization stack for agentic access and includes prompt injection prevention as an explicit research question alongside agent identity binding, least privilege, and non-repudiation.
