...
![Diagonalization diagram in three zones: Left (blue box) shows a FINITE RED-TEAM SUITE listing DAN, CRESCENDO, PAIR, SKELETON KEY with checkmarks. A large orange arrow labeled 'DIAGONALIZATION: CONSTRUCT NEW BYPASS FROM ANY FINITE LIST' breaks out of the blue box rightward, annotated 'FOR ANY FINITE LIST, A NEW BYPASS EXISTS.' Right zone shows a red cloud labeled UNCOVERED ATTACKS with an infinity symbol below and text 'COUNTABLY INFINITE RESIDUAL.' Below the full diagram, a wide green arrow points right labeled 'CONTINUOUS MONITORING PROGRAM — the only viable response.'](../img/adversarial-incompleteness-result.png)
*The Vassilev result (NIST, 2025): the diagonalization construction guarantees that any finite test suite leaves an infinite residual. The certification frame — red-team once, pass, done — is wrong at the structural level. Continuous monitoring is the only frame that fits the math.*
